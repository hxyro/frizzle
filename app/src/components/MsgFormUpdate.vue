...
// Form data.
const content = ref(msg.value.content);
const topic = ref(msg.value.topic);
...
const canMsg = computed(() => content.value && characterLimit.value > 0);

// Actions.
const emit = defineEmits(["close"]);
const update = async () => {
  if (!canMsg.value) return;
  await updateMsg(msg.value, slugTopic.value, content.value);
  emit("close");
};
</script>
...
<template>
  <div v-if="connected">
    <div class="px-8 py-4 border-l-4 border-green-600">
      <div class="py-1">
      ...
      <textarea
        ref="textarea"
        rows="1"
        class="text-xl w-full focus:outline-none resize-none mb-3 bg-gray-800 p-4 rounded-xl"
      ...
      <div class="flex flex-wrap items-center justify-between -m-2">
        ...
        <div class="relative m-2 mr-4">
          <input
            type="text"
            placeholder="topic"
            class="text-green-500 rounded-full pl-10 pr-4 py-2 bg-gray-800"
            :value="slugTopic"
          ...
          <div class="absolute left-0 inset-y-0 flex pl-3 pr-2">
            <svg
              ...
              :class="slugTopic ? 'text-green-500' : 'text-gray-400'"
              viewBox="0 0 20 20"
              fill="currentColor"
            >
              <path
  ...
  <div v-else class="px-8 py-4 bg-gray-50 text-gray-500 text-center border-b">
    Connect your wallet to start msging...
  </div>
</template>
